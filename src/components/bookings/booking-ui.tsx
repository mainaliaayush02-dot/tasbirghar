...
const COPY: Record<BookingAction, { label: string; title: string; body: string; danger: boolean }> = {
  ...
  studio_cancel: { label: "Cancel booking", title: "Cancel this confirmed booking?", body: "The customer will see that the studio cancelled, and the time becomes free again. Please also contact the customer directly.", danger: true },
};
      ...
      {actions.map((action) => {
        ...
        return (
          <ConfirmDialog
            ...
            trigger={(open) => (
              <Button size="sm" variant={c.danger ? "danger" : action === "confirm" || action === "complete" ? "primary" : "secondary"} onClick={open}>
                {c.label}
              </Button>
            )}
          />
